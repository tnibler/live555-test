...
Encoder::Encoder(UsageEnvironment &env) : usage_env(env), h(NULL) {
  int i = 0;
...
void Encoder::getFromEncoder() {}

bool Encoder::isNalsAvailableInOutputQueue() {
  ...
  for (int i = 0; i < mmap_buffer_cnt; i++) {
    ...
    buffers[i].start =
        mmap(NULL /* start anywhere */, buf.length,
             ...
             MAP_SHARED /* recommended */, video_dev_desc, buf.m.offset);
...
void Encoder::yuyv(const void *p, int size) {
  int i, j;
  static int a = 0;
  a++;
  x264_param_t param;
  ...
  for (j = 0; j != VIDEO_WIDTH * VIDEO_HEIGHT / 4; j++) {
    ...
    {
      pic.img.plane[1][j] = (frame3->YUYVSource[4 * j + 1] +
                             frame3->YUYVSource[row + 4 * j + 1]) /
                            2;

      pic.img.plane[2][j] = (frame3->YUYVSource[4 * j + 3] +
                             frame3->YUYVSource[row + 4 * j + 3]) /
                            2;
    }
  }
  pic.i_pts = i_frame;
  ...
  while (x264_encoder_delayed_frames(h)) {
    ...
    if (i_frame_size > 0) {
      int i = 0;
      for (; i < MAX_NAL_COUNT; i++) {
        memcpy(nal_references[i]->p_payload, nal[i].p_payload,
               nal[i].i_payload);
        nal_references[i]->i_payload = nal[i].i_payload;
        outputQueue.push(nal_references[i]);
      }
...
void Encoder::process_image(const void *p, int size) { yuyv(p, size); }

void Encoder::x264_print_err(const char *s) {
  usage_env << s << " " << errno << "\n" << strerror(errno) << "\n";
}

int Encoder::read_frame(void) {
  struct v4l2_buffer buf;
  //	unsigned int i;
...
int Encoder::x264_ioctl(int fh, unsigned long int request, void *arg) {
  int ret;
